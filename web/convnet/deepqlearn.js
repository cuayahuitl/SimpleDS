...
(function(global) {
  ...
  var Brain = function(num_states, num_actions, opt) {
    ...
    // action sets
    this.allowed_actions_array = [];
    //this.allowed_actions_table = {};
  ...
  Brain.prototype = {
    random_action: function() {
      ...
      if(this.random_action_distribution.length === 0) {
        var index = parseInt(Math.random()*this.allowed_actions_array.length);
        var action = this.allowed_actions_array[index];
        return action;
      } else {
    ...
    policy: function(s) {
      ...
      return {action:maxkey, value:maxval};
    },
    ...
    qvalue: function(input_array, index) {
      if(this.learning) {
        var net_input = this.getNetInput(input_array);
        var svol = new convnetjs.Vol(1, 1, this.net_inputs);
        svol.w = net_input;
        var action_values = this.value_net.forward(svol);
        console.log(action_values.w);
        return action_values.w[index];
      }
    },
    allowed: function(action) {
      var value = this.allowed_actions_table[action];
      var allowed = (value == undefined) ? false : true;
      return allowed;
    },
